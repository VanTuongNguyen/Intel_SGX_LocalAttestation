...
Encl1_C_Files := $(TRUSTED_DIR)/encl1.c 
Encl1_Include_Paths := -IInclude -I$(TRUSTED_DIR) -I$(SGX_SDK)/include -I$(SGX_SDK)/include/tlibc -I$(SGX_SDK)/include/libcxx
...
Encl1_Link_Flags := $(SGX_COMMON_CFLAGS) -Wl,--no-undefined -nostdlib -nodefaultlibs -nostartfiles -L$(SGX_LIBRARY_PATH) \
	...
	-Wl,--version-script=$(TRUSTED_DIR)/encl1.lds
...
$(TRUSTED_DIR)/encl1_t.c: $(SGX_EDGER8R) ./$(TRUSTED_DIR)/encl1.edl
	@cd ./$(TRUSTED_DIR) && $(SGX_EDGER8R) --trusted ../$(TRUSTED_DIR)/encl1.edl --search-path ../$(TRUSTED_DIR) --search-path $(SGX_SDK)/include
	@echo "GEN  =>  $@"

$(TRUSTED_DIR)/encl1_t.o: ./$(TRUSTED_DIR)/encl1_t.c
...
encl1.so: $(TRUSTED_DIR)/encl1_t.o $(Encl1_C_Objects)
	@$(CXX) $^ -o $@ $(Encl1_Link_Flags)
	@echo "LINK =>  $@"

encl1.signed.so: encl1.so
	@$(SGX_ENCLAVE_SIGNER) sign -key $(TRUSTED_DIR)/encl1_private.pem -enclave encl1.so -out $@ -config $(TRUSTED_DIR)/encl1.config.xml
	@echo "SIGN =>  $@"
clean:
	@rm -f encl1.* $(TRUSTED_DIR)/encl1_t.*  $(Encl1_C_Objects)